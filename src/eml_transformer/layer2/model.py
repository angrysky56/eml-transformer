...
from eml_transformer.models.layers import FeedForward, CausalSelfAttention
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: Tensor, context: Tensor) -> Tensor:
        ...
        kv = self.kv_proj(context).view(batch_size, context_len, 2, self.n_heads, self.head_dim).transpose(1, 3)
...
class Layer2DecoderLayer(nn.Module):
    ...
    def __init__(self, cfg: Layer2Config):
        ...
        self.self_attn = CausalSelfAttention(cfg.d_model, cfg.n_heads, cfg.max_target_length)
    ...
    def forward(self, x: Tensor, context: Tensor, key_padding_mask: Tensor | None = None) -> Tensor:
...
class SignatureProgramModel(nn.Module):
    ...
    def __init__(self, cfg: Layer2Config):
        ...
        self.decoder_layers = nn.ModuleList([Layer2DecoderLayer(cfg) for _ in range(cfg.n_decoder_layers)])
