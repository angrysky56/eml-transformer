...
from eml_transformer.layer2.torch_dataset import (
    SignatureProgramDataset,
    collate_signature_fn,
)
...
def _collect_vars(node, v_set):
    """Recursively collect variable names from an EML tree."""
    if node.kind is TokenKind.VAR:
        v_set.add(node.var_name)
    elif node.kind is TokenKind.EML:
        _collect_vars(node.left, v_set)
        _collect_vars(node.right, v_set)


@torch.no_grad()
def evaluate(
    model: SignatureProgramModel,
    loader: DataLoader,
    ...
    for batch in loader:
        ...
        for i, pred_ids in enumerate(gen_ids):
            target_rpn = tokenizer.decode(
                labels[i].tolist()
            )  # Note: labels has -100 for pad, decode needs to handle it or we use input_ids
            # Cleaner: decode input_ids but strip BOS/PAD
            target_ids = input_ids[i].tolist()
            # target_ids has BOS and PAD.
            target_rpn = tokenizer.decode(
                [t for t in target_ids if t not in (tokenizer.pad_id, tokenizer.bos_id)]
            )
            ...
            if pred_rpn == target_rpn:
                ...
                try:
                    ...
                    target_sig = [
                        complex(r.item(), j.item()) for r, j in target_sig_complex
                    ]
                    ...
                    if tree:
                        ...
                        # Compare
                        is_match = True
                        for p_val, t_val in zip(pred_sig, target_sig, strict=False):
    ...
    return (
        ...
        total_exact_match / total_count,
        total_sig_match / total_count,
    )
...
def train():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Ratio of depth-2 to include in training (0.0 = pure generalization test)",
    )
    parser.add_argument(
        "--max_samples",
        type=int,
        default=100000,
        help="Cap total dataset size for speed",
    )
    parser.add_argument(
        "--max_depth", type=int, default=2, help="Max composition depth for generation"
    )
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        collate_fn=collate_signature_fn(tokenizer.pad_id),
    )
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch == args.epochs - 1:
            val_loss, exact_match, sig_match = evaluate(
                model, val_loader, tokenizer, device
            )
            print(
                f"Val Loss: {val_loss:.4f} | Exact Match: {exact_match:.2%} | Sig Match: {sig_match:.2%}"
            )
